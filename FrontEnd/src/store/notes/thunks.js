...
export const getNote = createAsyncThunk('note/getNote', async (id, { dispatch }) => {
	try {
		const token = localStorage.getItem('token');
...
export const createNote = createAsyncThunk('note/createNote', async (noteo, { dispatch }) => {
	try {
		const token = localStorage.getItem('token');
		const res = await apiURL.post('/api/notas', noteo, {
			withCredentials: true,
			headers: { authorization: `Bearer ${token}` },
		});
		getNotes();
...
export const deleteNote = createAsyncThunk('note/deleteNote', async (id, { dispatch }) => {
	try {
